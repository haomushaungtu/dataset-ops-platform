...
部署清单核对日期：2026-09-04。本文件只记录能力选择和只读核验结论，不复制密码、密钥或完整含凭据连接串。任何资源创建前必须获得目标命名和所有者确认。
...
| OpenSearch | 3.4.0 Min，独立账号 `dfsearch`，回环端口 `19200/19300`，768 MiB 堆；OpenMetadata 1.13.0 创建 60 个索引和 61 个模板，合成实体搜索及删除通过 | 2026-09-03 验收后曾停机，2026-09-04 已按原生非容器方式重新运行；重启后 OpenMetadata `18585/18586`、适配器 `19110/19111`、OpenSearch `19200` 正常且 `verify` 门禁通过，systemd 保持 `disabled`。无安全插件的 Min 配置只限本机 PoC，生产必须启用认证/TLS |
...
| Nacos / IAM | Nacos 2.5.1 健康；既有 `hqd-auth.jar` 不是可直接接入的标准 IdP。自建 `backend/identity-service` 已在 139 提供标准 discovery/JWKS，并签发 OpenMetadata 服务账号机器令牌 | 机器令牌已通过 OpenMetadata 验签并完成 API 访问；浏览器交互式登录、角色/团队收敛、禁用传播和全局登出仍待验收，不能据此判定统一登录全部完成 |
